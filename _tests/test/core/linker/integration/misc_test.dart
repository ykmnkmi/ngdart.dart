...
void main() {
  ...
  test('should allow variables in for loops', () async {
    final testBed = NgTestBed<VarInLoopComponent>(
      ng.createVarInLoopComponentFactory(),
    );
  ...
  test('should remove script tags from templates', () async {
    final testBed = NgTestBed<UnsafeComponent>(
      ng.createUnsafeComponentFactory(),
    );
    final testFixture = await testBed.create();
...
@Component(
  ...
  template:
      '<template ngFor [ngForOf]="list" let-i>'
...
@Directive(selector: '[update-host-attributes]')
...
@Directive(selector: '[update-host-properties]')
...
class DynamicViewport {
  ...
  DynamicViewport(ViewContainerRef vc) {
    ...
    final factoryFuture = Future<ComponentFactory<ChildCompUsingService>>.value(
      ng.createChildCompUsingServiceFactory(),
    );
    done = factoryFuture.then(
      (componentFactory) => vc.createComponent(componentFactory, 0, injector),
    );
  }
}
