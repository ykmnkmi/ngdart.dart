    ...
    group('local', () {
      ...
      test('directive should be accessible via a query', () async {
        final testBed = NgTestBed<ShouldQueryDirective>(
          ng.createShouldQueryDirectiveFactory(),
        );
      ...
      test('directive should be injectable on same element', () async {
        final testBed = NgTestBed<ShouldInjectFromElement>(
          ng.createShouldInjectFromElementFactory(),
        );
        final testFixture = await testBed.create();
      ...
      test('directive should be injectable in same view', () async {
        final testBed = NgTestBed<ShouldInjectFromView>(
          ng.createShouldInjectFromViewFactory(),
        );
        final testFixture = await testBed.create();
        expect(testFixture.assertOnlyInstance.child!.directive, isNotNull);
      });
      ...
      test('directive should inject host component', () async {
        final testBed = NgTestBed<ShouldInjectHost>(
          ng.createShouldInjectHostFactory(),
        );
        final testFixture = await testBed.create();
      ...
      test('component may provide itself via another token', () async {
        final testBed = NgTestBed<ShouldInjectAliasedLocal>(
          ng.createShouldInjectAliasedLocalFactory(),
        );
        final testFixture = await testBed.create();
      ...
      test('directive may provide itself for a multi-token', () async {
        final testBed = NgTestBed<ShouldInjectMultiToken>(
          ng.createShouldInjectMultiTokenFactory(),
        );
        final testFixture = await testBed.create();
      ...
      test('should support $FactoryProvider', () async {
        final testBed = NgTestBed<ShouldSupportFactoryProvider>(
      ...
      test('should support $ClassProvider', () async {
        final testBed = NgTestBed<ShouldSupportClassProvider>(
...
@Component(selector: 'injects-visibility-local', template: '')
...
@Directive(selector: '[visibility-none]')
...
@Component(selector: 'injects-directive', template: '')
...
@Component(
  ...
  ''',
  directives: [InjectsDirectiveHostComponent, VisibilityNoneDirective],
)
class ShouldFailToInjectFromParentView {}

@Component(selector: 'visibility-local', template: '')
...
@Component(
  ...
  providers: [ExistingProvider(Dependency, ShouldInjectAliasedLocal)],
)
...
@Component(selector: 'injects-aliased-local', template: '{{dependency.text}}')
...
@Component(selector: 'injects-visibility-all', template: '')
...
@Directive(
  selector: '[all]',
  providers: [
    ExistingProvider.forToken(implementations, VisibilityAllImplementation),
  ],
  visibility: Visibility.all,
)
...
@Directive(
  selector: '[local]',
  providers: [
    ExistingProvider.forToken(implementations, VisibilityLocalImplementation),
  ],
)
class VisibilityLocalImplementation implements Interface {}

@Component(selector: 'injects-multi-token', template: '')
...
@Component(
  ...
  directives: [ShouldInjectInterface],
  providers: [FactoryProvider(Interface, getInterfaceFromImpl)],
)
...
@Component(selector: 'should-inject-interface', template: '')
...
@Component(
  ...
  template: '<should-inject-interface></should-inject-interface>',
  directives: [ShouldInjectInterface],
  providers: [ClassProvider(Interface, useClass: ShouldSupportClassProvider)],
)
